...
`canvas-plumbing`. Painting is on the window. A saved shape is a
query, not a snapshot. Rails read the tree. `scripts/canvas_model.py`
holds the query contract; `python3 scripts/test_canvas_model.py`
fails if a shape carries tree data. Nothing on the canvas writes to
the tree.
